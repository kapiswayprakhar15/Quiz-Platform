...
function changeLanguage() {
  var selectElement = document.getElementById("options_menu");
  var codeRunnerElement = document.getElementById("change_demo");
  var selectedLanguage = selectElement.value;

  // Set the selected language for the code runner
  codeRunnerElement.setAttribute("language", selectedLanguage);
}
...
<pre style="padding: 0px; --bg:rgb(58, 54, 54); --text:rgb(255, 255, 255); --border:rgba(0,0,0,0.3); --code:rgb(255, 255, 255); --code-bg:rgba(39, 40, 35, 1); --title:rgb(255, 255, 255); --button-text:wheat; --button-border:rgba(0,0,0,0.18);width:50vw;top:10vh"><div class="code-knack-playground "><div class="code-knack-pane"><div class="code-knack-title">Error!</div>
    ...
    <!--compress-->
  <div class="code-runner-component" language="${language}">
  
          
  
<pre style="padding: 0px; --bg:rgb(58, 54, 54); --text:rgb(255, 255, 255); --border:rgba(0,0,0,0.3); --code:rgb(255, 255, 255); --code-bg:rgba(39, 40, 35, 1); --title:rgb(255, 255, 255); --button-text:wheat; --button-border:rgba(0,0,0,0.18);width:50vw;top:10vh"><div class="code-knack-playground " data-lang="cpp" data-options=""><div class="code-knack-pane"><div class="code-knack-title">${language}</div>
  <div class="code-knack-mask">Copied to the clipboard.</div>
<div class="code-knack-ctrl">


<button class="ck-button run-button" code-runner-button><img src="https://lyricat.github.io/code-knack/demo/lib/code-knack/images/icon-play-dark.svg"><span >run</span></button></div>
...
async function runCode(codeRunner) {
    // Get the editor instance
    const editor = ace.edit(codeRunner.querySelector("#codetorun"));
    ...
    try {
        await getData(codeRunner);
    } catch (error) {
        // Display a generic syntax error message in the output section
        codeRunner.querySelector("#result").innerText = "Syntax error: There is a problem with your code.";
...
let gfgScript = ['https://cdn.jsdelivr.net/npm/ace-min-noconflict@1.1.9/ace.min.js', 'https://cdn.jsdelivr.net/npm/ace-min-noconflict@1.1.9/ext-modelist.js', "https://cdn.jsdelivr.net/npm/ace-min-noconflict@1.1.9/ext-themelist.js", 'https://cdn.jsdelivr.net/npm/ace-min-noconflict@1.1.9/ext-language_tools.js']
let promiseData = [];
...
function CreateAceCodeEditor(html_element, language) {
    ...
    if (typeof CodeRunner_AceEditor_Theme == "undefined") {
        ...
        let themes = themelist.themesByName;
    ...
    // Set the initial content to 15 empty lines
    editor.setValue(Array(15).fill('').join('\n'));
